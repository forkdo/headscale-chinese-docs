...
我们使用 [GitHub Milestones 来规划即将发布的 Headscale 版本](https://github.com/juanfont/headscale/milestones)。
查看 [我们当前的计划](https://github.com/juanfont/headscale/milestones) 可以了解某个具体功能何时会
被实现。发布计划可能随时更改。

如果您有兴趣贡献，请提交一个功能请求。请注意，我们可能拒绝特定贡献的原因有很多：
...
请按照[升级指南](../setup/upgrade.md)中的步骤更新现有的 Headscale 安装。如果您落后多个版本，必须每次更新一个稳定版本（例如 0.26.0 → 0.27.1 → 0.28.0），不得跳中间的次要版本。您应始终选择最新的补丁版本。
...
## 为什么两个节点在彼此的状态中可见，即使策略规则只允许单向流量？

一个常见的用例是只允许从一个节点到另一个节点的流量，而不允许反向流量。例如，管理员的工作站应能连接到所有节点，但节点本身不应能连接回管理员的设备。为什么所有节点在 `tailscale status` 的输出中都能看到管理员的工作站？

这本质上就是 Tailscale 的工作方式。如果允许单向流量，则两个节点在各自的 `tailscale status` 输出中都能看到彼此。流量仍然根据策略进行过滤，唯一的例外是 `tailscale ping`，它始终在两个方向都允许。

另请参阅 <https://tailscale.com/docs/concepts/device-visibility>。

## 我的策略存储在数据库中，且由于策略无效，Headscale 拒绝启动。如何恢复？

Headscale 在启动时会检查策略是否有效，如果检测到错误则拒绝启动。错误信息会指出策略中哪一部分无效。请按照以下步骤修复策略：

- 将策略导出到文件：`headscale policy get --bypass-server-and-access-database-directly > policy.json`
- 编辑并修复 `policy.json`。使用命令 `headscale policy check --file policy.json` 验证策略。
- 加载修改后的策略：`headscale policy set --bypass-server-and-access-database-directly --file policy.json`
- 按常规方式启动 Headscale。
...
Tailscale 仅支持 IP 前缀 `100.64.0.0/10` 和 `fd7a:115c:a1e0::/48` 或其更小的子网。可使用以下步骤从不受支持的 IP 前缀迁移回受支持且推荐的前缀。

!!! warning "需要备份并在演示环境中测试"

    以下命令会更新您 tailnet 中所有节点的 IP 地址，这可能对您特定的环境造成严重影响。至少应做到：

    - [为您的数据库创建备份](../setup/upgrade.md#backup)
    - 在具有代表性的演示环境中测试以下命令。这有助于尽早发现后续的连接错误，并了解 tailnet 在您特定环境下的行为。

- 停止 Headscale
- 在[配置文件](../ref/configuration.md)中恢复默认前缀：
    ```yaml
    prefixes:
      v4: 100.64.0.0/10
      v6: fd7a:115c:a1e0::/48
    ```
- 更新数据库中的 `nodes.ipv4` 和 `nodes.ipv6` 列，并为每个节点分配唯一的 IPv4 和 IPv6 地址。以下 SQL 语句基于节点 ID 分配 IP 地址：
    ```sql
    UPDATE nodes
    SET ipv4=concat('100.64.', id/256, '.', id%256),
        ipv6=concat('fd7a:115c:a1e0::', format('%x', id));
    ```
- 更新[策略](../ref/policy.md)以反映 IP 地址的变更（如有）
- 启动 Headscale

节点应在几秒内重新连接并获取其新分配的 IP 地址。

## 如何避免将日志发送给 Tailscale Inc？

Tailscale 客户端[收集有关其操作和与其他客户端连接尝试的日志](https://tailscale.com/docs/features/logging#client-logs)，并将其发送到由 Tailscale Inc 运营的中心日志服务。
...
此外，也可以在客户端禁用日志记录。这与 Headscale 无关，退出客户端日志记录会在客户端启动早期就禁用日志提交。配置因操作系统而异，通常通过设置环境变量 `TS_NO_LOGS_NO_SUPPORT=true` 或向 `tailscaled` 传递标志 `--no-logs-no-support` 来实现。详见 <https://tailscale.com/docs/features/logging#opt-out-of-client-logging> 了解详情。